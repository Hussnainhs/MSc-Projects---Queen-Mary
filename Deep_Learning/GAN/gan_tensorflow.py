...
def discriminator(x, drop_out):
    ...
    h0_leaky_relu = tf.nn.leaky_relu(tf.matmul(x, w0) + b0)
    h0 = tf.nn.dropout(h0_leaky_relu, drop_out)

    # 2nd hidden layer
    w1 = tf.get_variable('D_w1', [h0.get_shape()[1], 512], initializer=w_init)
    b1 = tf.get_variable('D_b1', [512], initializer=b_init)
    h1_leaky_relu = tf.nn.leaky_relu(tf.matmul(h0, w1) + b1)
    h1 = tf.nn.dropout(h1_leaky_relu, drop_out)

    # 3rd hidden layer
    w2 = tf.get_variable('D_w2', [h1.get_shape()[1], 256], initializer=w_init)
    b2 = tf.get_variable('D_b2', [256], initializer=b_init)
    h2_leaky_relu = tf.nn.leaky_relu(tf.matmul(h1, w2) + b2)
    h2 = tf.nn.dropout(h2_leaky_relu, drop_out)
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    out_img = sess.run(G_z, {z: z_, drop_out: 0.0})

    figure_grid = 5
    fig, axes = plt.subplots(figure_grid, figure_grid, figsize=(5, 5))
    for i, j in itertools.product(range(figure_grid), range(figure_grid)):
    ...
    for a in range(5*5):
        ...
        j = a % 5
        axes[i, j].cla()
        axes[i, j].imshow(np.reshape(out_img[a], (28, 28)), cmap='gray')

    fig.text(0.5, 0.9, 'Epoch {0}'.format(num_epoch), ha='center')
    plt.savefig(path)
